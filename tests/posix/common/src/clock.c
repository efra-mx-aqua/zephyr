...
void test_clock_gettime_rollover(void)
{
	uint64_t t;
	...
	zassert_ok(clock_gettime(CLOCK_MONOTONIC, &ts[0]), NULL);
	ts_print("t-1", &ts[0]);
	zassert_equal(rollover_s, ts[0].tv_sec, NULL);

	t = UINT64_MAX;
	/* align to tick boundary */
	k_sleep(K_TICKS(1));
	sys_clock_tick_set(t);
	zassert_ok(clock_gettime(CLOCK_MONOTONIC, &ts[1]), NULL);
	ts_print("t+0", &ts[1]);
	zassert_equal(rollover_s, ts[1].tv_sec, NULL);
	zassert_true(ts_gt(&ts[1], &ts[0]), NULL);

	t = UINT64_MAX + 1;
	/* align to tick boundary */
	k_sleep(K_TICKS(1));
	sys_clock_tick_set(t);
	zassert_ok(clock_gettime(CLOCK_MONOTONIC, &ts[2]), NULL);
	ts_print("t+1", &ts[2]);
	zassert_equal(0, ts[2].tv_sec, NULL);
	zassert_true(ts_gt(&ts[1], &ts[2]), NULL);
}
